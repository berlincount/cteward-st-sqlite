...
def open_database(dbname):
    # if no dbname was given in the request itself, use start of hostname requested
    if not dbname:
...
@app.route("/entries",          methods=['GET'], defaults={'dbname': None})
@app.route("/<dbname>/entries", methods=['GET'])
...
# create an entry
@app.route("/entries",          methods=['POST'], defaults={'dbname': None})
@app.route("/<dbname>/entries", methods=['POST'])
...
# get a single entry
@app.route("/entries/<string:entry_id>",          methods=['GET'], defaults={'dbname': None})
@app.route("/<dbname>/entries/<string:entry_id>", methods=['GET'])
...
@app.route("/entries/<string:entry_id>",          methods=['PUT'], defaults={'dbname': None})
@app.route("/<dbname>/entries/<string:entry_id>", methods=['PUT'])
...
@app.route("/entries/<string:entry_id>",          methods=['PATCH'], defaults={'dbname': None})
@app.route("/<dbname>/entries/<string:entry_id>", methods=['PATCH'])
...
@app.route("/entries/<string:entry_id>",          methods=['DELETE'], defaults={'dbname': None})
@app.route("/<dbname>/entries/<string:entry_id>", methods=['DELETE'])
...
@app.route("/",          methods=['GET'], defaults={'dbname': None})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run the Flask cteward-st-sqlite app')
